...
You can download the data for this exercise as an R-data file [here](https://clausrueffler.github.io/svensk-fageltaxering/bird_data_subset2023.RDA). 
Alternatively, you can download the data as a *.csv* [here](https://clausrueffler.github.io/svensk-fageltaxering/bird_data_subset2023.csv). 
...
In this data set, each row (or, each *observation*) represents the count of a species in a given year on a given survey route.  The first row, for example, says that there was 4 individual of *Alauda arvensis* (Skylark) observed in route number 3 during the year 1998.  Here's a description of the columns:
...
Notice, that this data frame lists first all farmland species and then all forest species. Second, to this simplified data frame, we want to add the total number of observations for each species. We use the `tapply` function to sum the counts of each species.
