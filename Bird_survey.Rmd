...
In this exercise you analyze some data from *The Swedish Bird Inventory (Svensk Fågeltaxering)*. On [this](https://www.fageltaxering.lu.se/) website, you can read more about the data set (field protocol, history, etc.). A second aim is to continue improving your R-skills. Note that this exercise consists of **two** tabs.
...
```{r,  eval=T}
# dimension of the data frame "birds" (number of rows and columns)
dim(birds)
...
```{r}
# set the layout and margins of the figure
...
Let us have a look at these graphs. Hm, it seems that most species tend to increase... Do you think that this is indeed true? And if you do not think so, what could be the reason for the trends shown in the graphs?

The general increase in total observations per species is at least partly due to the fact that the Swedish Bird Inventory added new survey routes during the first years. In the lecture you saw a graph showing that during the first ten years, the number of routes counted steadily increased from one year to the next. This is problematic for making valid comparisons between years because we are obviously likely to count more birds when we survey more routes (*think back to the lecture on* ***rarefaction***).
...
<span style="color: purple;">
**Question 2**: After correcting for number of routes sampled per year, which species show the most obvious increase in abundance over the survey period?  And which species show the most obvious decrease in abundance over the survey period? Can you detect a general trend among farmland and forest species?
</span>

For times series like these, it is common to report the data as an index relative to the count of the first year. Given the code that we have, it is easy to implement this change. All that we have to do is to divide the average number of individuals per route by the number counted in the first year. Let us have a look at the time series for the Skylark *Alauda arvensis*.
...
<span style="color: purple;">
**Question 3**: 
Let us compare our plots with those produced by Svensk Fågeltaxering. You can find the official time series plot from Svensk Fågeltaxering [here](https://www.fageltaxering.lu.se/resultat/populationstrender-och-forandringar) (Scroll down to the tab *Trender för punkt- och standardrutter*). Note that the species on this website are in taxonomic order, not alphabetic order.
